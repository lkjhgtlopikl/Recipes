...
export const EditRecipe = () => {
  ...
  const { data: stepsData } = trpc.getSteps.useQuery({ id: idrecipe }, { enabled: !!r });
  const { data: ingredientsData } = trpc.getIngredients.useQuery({ id: idrecipe }, { enabled: !!r });
  const { data: dataCuisines, isLoading: cuisinesLoading, error: cuisinesError } = trpc.getCuisines.useQuery();
  const { data: dataTypes, isLoading: typesLoading, error: typesError } = trpc.getTypes.useQuery();
  ...
  const cuisineOptions = dataCuisines?.map((c) => ({ value: String(c.cuisine_id), label: c.name })) ?? [];
  const typeOptions = dataTypes?.map((t) => ({ value: t.typeCooking_id, label: t.name })) ?? [];
  const categoryOptions = dataCategory?.map((c) => ({ value: c.category_id, label: c.name })) ?? [];
  const menuOptions = dataMenu?.map((m) => ({ value: m.menu_id, label: m.name })) ?? [];
  const ingredientOptions = dataIngredients?.map((i) => ({
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={formik.handleSubmit}>
        <Input name="title" type="text" label="Название рецепта:*" placeholder="Введите название" formik={formik} />
        ...
        <TextArea name="description" label="Описание:*" placeholder="Краткое описание" formik={formik} />
        ...
        <div className="form-row">
          <Input name="cookingTime" type="number" label="Время приготовления (мин):*" placeholder="30" formik={formik} />
          <Input name="servings" type="number" label="Количество порций:*" placeholder="4" formik={formik} />
        </div>
        <div className="form-row">
          <Select name="difficulty" label="Сложность:*" list={difficultyOptions} formik={formik} placeholder="Выберите сложность" />
          <Select name="cuisine_id" label="Кухня:*" list={cuisineOptions} formik={formik} placeholder="Выберите кухню" />
        </div>
        <div className="form-row">
          <Select name="typeCooking_id" label="Тип приготовления:*" list={typeOptions} formik={formik} placeholder="Выберите тип" />
        ...
        <div className="form-actions">
          <button
            ...
            className="btn btn-primary"
          >
            {updateRecipeMutation.isPending ? "Сохраняем..." : "Сохранить изменения"}
          </button>
          <a href="/recipes" className="btn btn-outline">Отмена</a>
        </div>
      </form>
    </div>
  );
